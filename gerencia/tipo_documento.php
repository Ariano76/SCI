...
<h1 class="display-8">MAESTRO ITEM TIPO DE DOCUMENTO</h1> 
<div class="container">
...
<script type="text/javascript">
  $(document).ready(function() {
    $('#tablaUsuarios').DataTable({
      ...
      'order': [],
      'ajax': {
        'url': 'tipo_documento_fetch_data.php',
        'type': 'post',
      },
      "aoColumnDefs": [{
        "bSortable": false,
  ...
  $(document).on('submit', '#addUser', function(e) {
    e.preventDefault();
    var nom_documento = $('#addnom_documentoField').val();
    if (nom_documento != '') {
      $.ajax({
        url: "tipo_documento_add.php",
        type: "post",
        data: {
  ...
  $(document).on('submit', '#updateUser', function(e) {
    e.preventDefault();
    var nom_documento = $('#nom_documentoField').val();
    var trid = $('#trid').val();
    var id = $('#id').val();
    if (nom_documento != '') {
    $.ajax({
      url: "tipo_documento_update.php",
      type: "post",
      data: {
        nom_documento: nom_documento,
        id: id
      ...
      success: function(data) {
        ...
        if (status == 'true') {
          ...
          row.row("[id='" + trid + "']").data([id, nom_documento, button]);
  ...
  $('#tablaUsuarios').on('click', '.editbtn ', function(event) {
    ...
    var trid = $(this).closest('tr').attr('id');
      ...
      $.ajax({
        url: "tipo_documento_get_single.php",
        data: {
          id: id
        },
        type: 'post',
        success: function(data) {
          var json = JSON.parse(data);
          $('#nom_documentoField').val(json.nom_documento);
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="updateUser">
            ...
            <div class="mb-3 row">
              <label for="nom_documentoField" class="col-md-3 form-label">Nombre Item</label>
              <div class="md-form amber-textarea active-amber-textarea col-md-8">
                <input type="text" class="form-control" id="nom_documentoField" name="tipoident" maxlength="50">
              </div>
            </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="addUser" action="">
            <div class="mb-3 row">
              <label for="addnom_documentoField" class="col-md-3 form-label">Item</label>
              <div class="col-md-9">
                <input type="text" class="form-control" id="addnom_documentoField" name="tipoident" maxlength="50">
              </div>
            </div>
            <div class="text-center">
