...
$db = new TransactionSCI();
  ...
  if ($gestante>0) {
    $usuarios = $db->select_repo_gerencia_gestante("SP_repo_gerencia_regiontrimestre_gestante",$gestante);
    $retVal = 'Gestante' . "_" . $txt1;
  } elseif ($discapacidad>0) {
    $usuarios = $db->select_repo_gerencia_gestante("SP_repo_gerencia_regiontrimestre_discapacidad",$discapacidad);
    $retVal = 'Discapacidad' . "_" . $txt2;
  } else {
    $usuarios = $db->select_repo_gerencia_gestante("SP_repo_gerencia_regiontrimestre_nacionalidad",$nacionalidad);
    $retVal = 'Nacionalidad' . "_" . $txt3;
  }
  $name = "Reporte_total_reach_regiontrimestre_".$retVal;
